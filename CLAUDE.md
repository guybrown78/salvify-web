...
**No em dashes anywhere.** Not in site copy, metadata, comments, docs or commit messages. Copy must read like a person wrote it: use full stops, commas, colons, parentheses or "and" instead. (The WIP v2 pages under `src/app/(site)/` and the Sanity schemas still contain older em dashes; clean them when you touch those files.)
...
Within a template, section content can select a specific visual component by string key (`heroComponentKey`, `problemComponentKey`, and so on) resolved against a registry map (e.g. `featureHeroMap` in `src/components/features/heroVisuals`). To add a variant, add the component to the registry; do not branch in the template.
...
- Fonts: `font-sans` maps to Poppins (`--font-poppins`, body default), `font-brand` maps to Nunito (`--font-nunito`, headings; `@layer base` forces `h1` to `h4` to `font-brand font-extrabold`).
...
- Known deferred cleanup is tracked in `docs/`, currently `docs/layout-and-404-todo.md` (nested `<html>`/`<body>` across root + route-group layouts; unbranded framework 404 fallback).
